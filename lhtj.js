...
const activity_no = $.isNode() ? process.env["lhtj_activity"] : $.getdata(ckName);
const component_no = $.isNode() ? process.env["lhtj_component"] : $.getdata(ckName);
console.log(activity_no, component_no);
...
async function lotteryClock(user) {
    try {
        const opts = {
            url: "https://gw2c-hw-open.longfor.com/llt-gateway-prod/v1/lottery/luck",
            headers: {
                'cookie': user.cookie,
                'user-agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.48(0x18003029) NetType/4G Language/zh_CN miniProgram/wx50282644351869da',
                'x-lf-usertoken': user.token,
